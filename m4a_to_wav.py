...
def get_m4a_files(directory):
    return [f for f in os.listdir(directory) if f.endswith(".m4a")]
...
def convert_all_m4a_to_wav(input_directory, output_directory):
    all_conversions_successful = True
    for filename in get_m4a_files(input_directory):
        ...
        try:
            ...
            print(f"Failed to convert {m4a_path} to {wav_path}. Error: {e}")
            all_conversions_successful = False
    return all_conversions_successful
default_directory = "../../Patients/"
print("Please enter the root directory containing m4a files to convert. Press Enter to use the default directory.")
...
print("Please enter a subdirectory or multiple subdirectories (separated by commas) of \033[94m" + default_directory + "\033[0m containing m4a files to convert. Example: CRP-01/D_Visit_1/Audio_recordings,CRP-01/D_Visit_2/Audio_recordings")
try:
...
for subdir in subdirs:
    ...
    if not etc_dir.startswith('CRP-'):
        print("\033[0;31mCould not extract CRP-XX from subdirectory. Please check your input.\033[0m")
        continue  # Skip to the next directory
    output_directory = "data/raw"

    try:
        m4a_files = get_m4a_files(input_directory)
        textgrid_files = get_textgrid_files(input_directory)

        all_conv_success = convert_all_m4a_to_wav(input_directory, output_directory)
